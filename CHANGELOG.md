8.0.0
   ...
   * In debug mode, a stale column cache for a class with no Grid annotation/attribute (e.g. grids configured via dtc_grid YAML files, whose cache is written at container compile time) is now served rather than silently replaced by reflection columns when the entity file is newer.
